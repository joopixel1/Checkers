 ...
 * @since 5.0
 */
public class MyWebSocketService implements WebSocketService, Lifecycle {
    ...
    static {
        ClassLoader classLoader = MyWebSocketService.class.getClassLoader();
    ...
    private static final Log logger = LogFactory.getLog(MyWebSocketService.class);
     ...
     */
    public MyWebSocketService(RequestUpgradeStrategy upgradeStrategy) {
    ...
    public Mono<Void> handleRequest(ServerWebExchange exchange, WebSocketHandler handler) {
        ...
        return exchange.getPrincipal()
                .flatMap(principal ->
                        initAttributes(exchange).flatMap(attribute -> this.upgradeStrategy.upgrade(exchange, handler, protocol,
                            () -> createHandshakeInfo(exchange, request, protocol, attribute, principal)))
                );
    }
    ...
    private HandshakeInfo createHandshakeInfo(ServerWebExchange exchange, ServerHttpRequest request,
                                              @Nullable String protocol, Map<String, Object> attributes, Principal principal) {
        ...
        return new HandshakeInfo(uri, headers, cookies, Mono.just(principal), protocol, remoteAddress, attributes, logPrefix);
    }
